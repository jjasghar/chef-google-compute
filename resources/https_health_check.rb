  ...
  module GCOMPUTE
    ...
    class HttpsHealthCheck < Chef::Resource
      ...
      action :create do
        ...
        if fetch.nil?
          converge_by "Creating gcompute_https_health_check[#{name}]" do
          ...
          cannot_change_resource 'HttpsHealthCheck cannot be edited'
        end
      end
      ...
      action :delete do
        fetch = fetch_resource(@new_resource, self_link(@new_resource),
                               'compute#httpsHealthCheck')
        unless fetch.nil?
          converge_by "Deleting gcompute_https_health_check[#{name}]" do
